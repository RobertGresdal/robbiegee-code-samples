...
class Neuron( af, threshold ) 
	#@param af The activation function. Needs x as parameter for process
	#@param threshold If af is greater or equal to threshold the neuron fires
	def initialize
		#~ if af.is_a(Proc) then
			#~ @af  = af
		#~ else
			#~ @af = $PROC_SIGMOID;#Proc.new do |x| {1/(1+Math.exp(-x)) }# Use sigmoid as default
			#~ #@af = af ? af : function(x){return 1/(1+Math.exp(-x))}; 
		#~ end
		@af = af.is_a(Proc) ? af : $PROC_SIGMOID
	...
	def fire(register)
		...
		if value >= this.threshold then
			#for(var i=0,end=this.connections.length;i<end;i++){
			@connections.length.times |i| do 
				c = @connections[i];
	...
	# @param register Proc
	# @param id the triggerID so we can figure out the fatigue
	# 
	# @todo implement the fatigue
	# @todo make into valid rubycode
	def trigger( value, register, id )
		@weights += value
		if not @triggerstate && register.is_a?(Proc) do
			register.call(self)
